import { motion } from "framer-motion";
import { Check } from "lucide-react";

const plans = [
  {
    name: "Starter UI",
    robux: "1,000 – 4,000",
    usd: "$5 – $15",
    note: "after discount",
    timeframe: "1 – 2 days",
    description: "Simple UI elements such as a menu, shop, settings, or a single HUD frame.",
    features: [
      "1 UI frame",
      "1 revision",
      "Basic styling",
      "Up to 2 frames per order",
    ],
    limit: "Extra frames charged separately",
    highlight: false,
  },
  {
    name: "Game UI Package",
    robux: "4,000 – 12,000",
    usd: "$15 – $50",
    note: "after discount",
    timeframe: "2 – 5 days",
    description: "A small connected UI system for gameplay — HUD, shop, and inventory combined into a consistent design.",
    features: [
      "Up to 4 UI frames",
      "2 revisions",
      "Consistent theme across all UI",
      "For early-stage or mid-size games",
    ],
    limit: "Large systems (rebirth/pets) charged separately",
    highlight: true,
  },
  {
    name: "Full Game UI Package",
    robux: "12,000 – 30,000",
    usd: "$50 – $100",
    note: "after discount",
    timeframe: "5 – 10 days",
    description: "Complete UI system including HUD, shop, inventory, rebirth, pets, and additional menus.",
    features: [
      "5+ UI screens",
      "Multiple revisions",
      "Full UI planning",
      "Consistent design system",
    ],
    limit: "Requires full UI list before starting",
    highlight: false,
  },
  {
    name: "Premium UI Package",
    robux: "30,000+",
    usd: "$100+",
    note: "after discount",
    timeframe: "1 – 3 weeks",
    description: "High-end UI systems for serious or large-scale games with strong visual identity and advanced layout design.",
    features: [
      "Full UI architecture design",
      "Priority communication",
      "Structured revision stages",
      "Optional animations",
    ],
    limit: "Requires clear project scope before starting",
    highlight: false,
  },
];

export function Pricing() {
  return (
    <section className="py-28 border-t border-white/5" id="pricing">
      <div className="max-w-7xl mx-auto px-6">
        <div className="max-w-xl mb-14">
          <p className="text-xs font-semibold uppercase tracking-widest gradient-text-accent mb-4">Pricing</p>
          <h2 className="text-4xl md:text-5xl font-bold text-white mb-4">Transparent pricing.</h2>
          <p className="text-white/45 text-base leading-relaxed">
            USD payments receive a <span className="text-white/70 font-semibold">20% discount</span> on the total price. Robux payments via Game Pass or Gift Card only.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {plans.map((plan, i) => (
            <motion.div
              key={i}
              initial={{ opacity: 0, y: 24 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: i * 0.08, duration: 0.5 }}
              className={`relative rounded-2xl p-6 flex flex-col ${plan.highlight
                ? 'border border-[rgba(72,0,255,0.5)] bg-[rgba(72,0,255,0.08)]'
                : 'glass'
              }`}
              data-testid={`pricing-card-${i}`}
            >
              {plan.highlight && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="text-[10px] font-bold uppercase tracking-widest text-white px-3 py-1 rounded-full"
                    style={{ background: 'linear-gradient(90deg, #7b5fff, #4800ff)' }}>
                    Most Popular
                  </span>
                </div>
              )}

              <div className="mb-5">
                <h3 className="text-base font-bold text-white mb-1">{plan.name}</h3>
                <p className="text-xs text-white/35 leading-relaxed">{plan.description}</p>
              </div>

              <div className="mb-5 pb-5 border-b border-white/8">
                <div className="flex items-baseline gap-1 mb-1">
                  <span className="text-xs text-white/30">R$</span>
                  <span className="text-xl font-bold text-white font-display">{plan.robux}</span>
                </div>
                <div className="flex items-center gap-1">
                  <span className="text-sm font-semibold gradient-text-accent">{plan.usd}</span>
                  <span className="text-xs text-white/30">{plan.note}</span>
                </div>
                <div className="mt-2 inline-flex items-center gap-1 px-2 py-0.5 rounded-full bg-white/5 border border-white/8">
                  <span className="text-[10px] text-white/40">Delivery:</span>
                  <span className="text-[10px] text-white/60 font-medium">{plan.timeframe}</span>
                </div>
              </div>

              <ul className="space-y-2 flex-1 mb-5">
                {plan.features.map((f, fi) => (
                  <li key={fi} className="flex items-start gap-2 text-xs text-white/55">
                    <Check className="w-3.5 h-3.5 text-[#7b5fff] flex-shrink-0 mt-0.5" />
                    {f}
                  </li>
                ))}
              </ul>

              <p className="text-[10px] text-white/25 leading-relaxed mb-5 border-t border-white/5 pt-4">
                {plan.limit}
              </p>

              <button
                onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
                className={`w-full py-2.5 rounded-xl text-sm font-semibold transition-all ${plan.highlight
                  ? 'text-white hover:opacity-90'
                  : 'bg-white/6 text-white/70 border border-white/10 hover:bg-white/10 hover:text-white'
                }`}
                style={plan.highlight ? { background: 'linear-gradient(90deg, #7b5fff, #4800ff)' } : {}}
                data-testid={`pricing-cta-${i}`}
              >
                Get Started
              </button>
            </motion.div>
          ))}
        </div>

        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          viewport={{ once: true }}
          className="mt-8 glass rounded-2xl p-5 flex flex-wrap items-center gap-6"
        >
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-violet-400" />
            <span className="text-sm text-white/50">Extra revisions: <span className="text-white/70">$3 each</span></span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-blue-400" />
            <span className="text-sm text-white/50">Rush delivery: <span className="text-white/70">+25% of original price</span></span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-purple-400" />
            <span className="text-sm text-white/50">Large projects: <span className="text-white/70">50% deposit upfront</span></span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 rounded-full bg-indigo-400" />
            <span className="text-sm text-white/50">Small projects: <span className="text-white/70">30% deposit upfront</span></span>
          </div>
        </motion.div>
      </div>
    </section>
  );
}
