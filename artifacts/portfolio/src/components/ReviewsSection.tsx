...
import { Star, Send, TrendingUp, Loader2, BadgeCheck, User, Gamepad2 } from "lucide-react";
    ...
    <div className="flex items-center gap-1">
      {[1, 2, 3, 4, 5].map(i => (
        <button key={i} type="button"
          ...
          onClick={() => onChange(i)} className="transition-transform hover:scale-125"
          style={{ color: i <= (hovered || value) ? "#f59e0b" : "rgba(255,255,255,0.15)" }}
        >
          <Star className="w-6 h-6" fill={i <= (hovered || value) ? "#f59e0b" : "none"} />
        </button>
      ))}
      {value > 0 && <span className="ml-2 text-sm text-white/50">{value}/5</span>}
    </div>
  );
}
...
function ReviewCard({ review, i }: { review: Review; i: number }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-1">
        {[1,2,3,4,5].map(s => (
          <Star key={s} className="w-3 h-3" fill={s <= review.rating ? "#f59e0b" : "none"}
            style={{ color: s <= review.rating ? "#f59e0b" : "rgba(255,255,255,0.1)" }} />
        ))}
      ...
      <div className="flex items-center gap-2 pt-2 border-t border-white/5">
        <div className="w-7 h-7 rounded-full flex items-center justify-center text-[10px] font-bold flex-shrink-0"
          style={{ background: "var(--c-glow-soft)", border: "1px solid var(--c-border)", color: "var(--c-primary)" }}>
          {initials}
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-white/60 leading-none truncate">{displayName}</p>
          {review.gameName && (
            <p className="text-[9px] text-white/25 mt-0.5 truncate">{review.gameName}</p>
          )}
        </div>
  ...
  const submit = async () => {
    ...
    try {
      const res = await fetch("/api/reviews", {
        ...
        body: JSON.stringify({ rating, comment: comment.trim(), username: username.trim(), gameName: gameName.trim() }),
      });
      if (!res.ok) { setError("Could not submit. Try again."); return; }
      setSubmitted(true);
      setRating(0); setComment(""); setUsername(""); setGameName("");
      await fetchReviews();
    } catch {
  ...
  return (
    <section className="py-28 border-t border-white/5 section-glow relative overflow-hidden" id="reviews" style={{ zIndex: 2 }}>
      <div className="absolute top-8 right-8 select-none pointer-events-none font-display font-bold leading-none"
        style={{ fontSize: "clamp(6rem,15vw,12rem)", color: "rgba(255,255,255,0.025)" }} aria-hidden="true">07</div>

      <div className="max-w-7xl mx-auto px-6 relative z-10">
        <div className="max-w-xl mb-14">
          <motion.p initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            className="text-xs font-semibold uppercase tracking-widest gradient-text-blue mb-4">Community Reviews</motion.p>
          <motion.h2 initial={{ opacity: 0, y: 20, filter: "blur(8px)" }} whileInView={{ opacity: 1, y: 0, filter: "blur(0px)" }}
            ...
            className="text-4xl md:text-5xl font-bold text-white mb-4">Rate the work.</motion.h2>
          <motion.p initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
            transition={{ delay: 0.15, duration: 0.6 }} className="text-base text-white/40 leading-relaxed">
            Leave an honest rating after your commission. Your username and game name help others trust your review.
          </motion.p>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            ...
            <div className="text-center">
              ...
              <div className="flex justify-center gap-0.5 mt-2">
                {[1,2,3,4,5].map(s => (
                  <Star key={s} className="w-4 h-4" fill={s <= Math.round(avg) ? "#f59e0b" : "none"}
                    style={{ color: s <= Math.round(avg) ? "#f59e0b" : "rgba(255,255,255,0.1)" }} />
                ))}
              </div>
              ...
              {submitted ? (
                ...
                <motion.div key="form" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div>
                      <label className="block text-xs text-white/30 mb-1.5 font-medium flex items-center gap-1">
                        <User className="w-3 h-3" /> Roblox username
                      </label>
                      <input value={username} onChange={e => setUsername(e.target.value)}
                        placeholder="Player123" maxLength={40}
                        className="w-full h-9 px-3 rounded-xl text-xs text-white placeholder:text-white/20 focus:outline-none"
                    ...
                    <div>
                      ...
                      <input value={gameName} onChange={e => setGameName(e.target.value)}
                        placeholder="My Game" maxLength={60}
                        className="w-full h-9 px-3 rounded-xl text-xs text-white placeholder:text-white/20 focus:outline-none"
                        style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.09)" }} />
                    </div>
                  </div>
                  <div>
                    ...
                    <textarea value={comment} onChange={e => setComment(e.target.value)}
                      placeholder="Share your experience..." rows={3} maxLength={300}
                  ...
                  <button onClick={submit} disabled={submitting || rating === 0}
                    className="h-11 rounded-xl font-semibold text-sm text-white flex items-center justify-center gap-2 btn-primary disabled:opacity-40 mt-auto">
          ...
          <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}
            ...
            <div className="flex flex-col gap-3 max-h-[340px] overflow-y-auto pr-1" style={{ scrollbarWidth: "none" }}>
              {loading && <div className="flex justify-center py-8"><Loader2 className="w-5 h-5 animate-spin text-white/30" /></div>}
              ...
              {data?.reviews.map((r, i) => <ReviewCard key={r.id} review={r} i={i} />)}
            </div>
          </motion.div>
        </div>
      </div>
